...
public class Sequence implements Iterable<Integer> {
  // set of constants that represent the sequence types supported 
  //by this sequence generator
  public enum SequenceType {
    ARITHMETIC, GEOMETRIC, FIBONACCI, DIGIT_PRODUCT
  };

  private Iterator<Integer> sequenceIterator; // iterator to 
   ...
   * 
   * @param command array of integers that represents 
   * the user command line to generate a sequence
   * 
   * @return: none
   * 
   */
  public Sequence(int[] command) {
    ...
    sequenceType = SequenceType.values()[command[0]]; 
    // set the sequence type
    switch (sequenceType) {
      case ARITHMETIC: 
        // call the constructor of the ArithmeticSequenceGenerator 
        //class to create
        // an iterator over an arithmetic sequence with 
        //initial element command[1], common
        // difference command[2], and size stored at command[3]
        sequenceIterator = 
        new ArithmeticSequenceGenerator(command[1], command[2], command[3]); 
        break;
      case GEOMETRIC:
        sequenceIterator = 
        new GeometricSequenceGenerator(command[1], command[2], command[3]);
        break;
      case FIBONACCI:
        sequenceIterator = 
        new FibonacciSequenceGenerator(command[1]);
        break;
      case DIGIT_PRODUCT:
        sequenceIterator = 
        new DigitProductSequenceGenerator
        (command[1], command[2]).getIterator();
        break;
    }
  }
   ...
   * @param: none
   * 
   * @return String that includes the sequence name and 
   * the different numbers of the sequence
   *         separated by a single space
   */
  @Override// Time Complexity: O(n) linear
  public String toString() {

    String seq = sequenceType.name() + " sequence: "; 
// initialize the String seq to return
    // Use a for-each loop to traverse the sequence 
    //and add the different numbers of the sequence
  ...
  /**
   * This helper method checks for the correctness of the syntax 
   * of the user command with respect to
   * the sequence type
   * 
   * @param userCommand command entered by the user
   * @return true if the userCommand contains a syntax error 
   * with respect to the program
   *         specification, false otherwise
   */
  private static boolean checkCommandSyntax(String[] userCommand) {
    // TODO add the time complexity of this method,
    //where the problem size N is the length
  ...
  @Override
  /**
   * This method is an accessor to the Iterator object
   * 
   * @param: none
   * 
  ...
  /**
   * main method that represents the driver application: 
   * starts the program, displays the command
   * menu, prompt the user for input, and displays 
   * the expected output
   * 
   * @param args
   * 
  ...
  public static void main(String[] args) {
    System.out.println(
        ...
        + "=================\r\n");
    // Set the menu, prompt the user command, error messages, and 
    //goodbye strings
    final String menu = "COMMAND MENU: \r\n" + " [Sequence_Code] "
        + "[Sequence_Parameters]\r\n"
        + "   [0 (for ARITHMETIC)  "
        + "] [First_Number Common_Difference Sequence_Size]\r\n"
        + "   [1 (for GEOMETRIC)   "
        + "] [First_Number Common_Ratio Sequence_Size]\r\n"
        + "   [2 (for FIBONACCI)   ] [Sequence_Size]\r\n"
        + "   [3 (for DIGIT PRODUCT SEQUENCE)] "
        + "[First_Number Sequence_Size]\r\n " + " \r\n"
        + " [Q]uit Program\r\n";
    final String promptUser = "\nENTER COMMAND: ";
    final String errorMsg = 
        "SYNTAX ERROR. Please refer to the above COMMAND MENU for "
        + "details.";
    final String formatErrMsg = 
        "ERROR: COMMAND must contain ONLY space separated integer "
        + "values";
    final String goodBye =
        "===================   Thank you for using "
        + "this App!!!!   ===================";

    boolean syntaxError = false; // boolean variable that tells 
    //if the syntax of the user command is
                                 // correct
    Sequence sequence; // sequence object
    Scanner input = new Scanner(System.in); 
    // Scanner object to read the user input
                                            
    // from the keyboard

    int[] seqCommand; // array of integers that represents 
    ...
    while (Character.toUpperCase(firstChar) != 'Q') {
      ...
      if (!syntaxError) {
        ...
        try {
          ...
          // generate the sequence with respect to 
          //the processed user command line
